<template>
    <div class="w-full p-10">
        <div class="border p-6 rounded mb-4 w-full shadow-[#072C5C]-500/50 shadow-lg">
            <h1 class="font-md text-xl">Refer new members and start enjoying lots of benefits</h1>
            
            <div data-v-684bf526="" class="w-full px-10 py-3 mb-5 rounded-[16px] border border-[#072C5C] bg-[#4f7ef343] text-[#1E1E1E] text-[14px] font-[400] flex justify-between items-center">
                <span>{{ this.refferal_link }}</span>
                <button @click="copyNow()" class="hover:transition-all hover:bg-[#0042EC] text-white  bg-[#0042EC] focus-visible:outline-none font-normal mt-2 leading-[150%] rounded-[0.5rem] px-2 h-8 text-center">Copy</button>
            </div>
            <span>
                To receive incentives and other perks for each new client brought in through your referral links, copy and distribute the referral link.
            </span>
        </div>

        <div  class="overflow-x-scroll mt-5 w-full">
            <h3  class="text-[20px] font-semibold leading-[21px]">Customers</h3>

            <div  v-if="customerData.length == 0" class="bg-[#D2DDFD] flex flex-col h-80 items-center justify-center my-10 rounded-lg w-full">
                ...
                <p class="text-[15px] font-semibold leading-[21px] font-[#1D2939]">No Customers</p>
            </div>

            <table  v-if="customerData.length != 0" class="w-full">
                <thead>
                    <tr class="p-6 border-b-2 border-b-[#B8B8B8] bg-[#D2DDFD]">
                        ...
                        <td class="text-[15px] font-semibold leading-[21px] font-[#1D2939] py-4">Customer Name</td> 
                        <td  class="text-[15px] font-semibold leading-[21px] font-[#1D2939] py-4">Date</td>
                    </tr>
                </thead>
                <tbody>
                    <tr class="cusor-auto p-6 border-b-2 hover:bg-[#D2DDFD]">
                        <td class="text-[15px] pl-5 font-normal leading-[21px] font-[#1D2939] py-4">1</td> 
                        <td class="text-[15px] font-bold leading-[21px] font-[#1D2939] py-4">-₦2,100.00</td> 
                        <td class="text-[15px] font-normal leading-[21px] font-[#1D2939] py-4"> 09/30/2023 09:15 </td>
                    </tr>
                </tbody>
...
export default {
  ...
  methods: {
    ...
    listDowliners() {
        ...
        this.$apiservice.listDowliners(param).then((data) => {
            this.customer = data.data.customer
            this.customerData = data.data.customer.data
  ...
  created() {
    this.listDowliners()
    this.user = this.$auth.user?.referral_code
    this.refferal_link = process.env.APP_URL+`?refferal_code=${this.$auth.user?.referral_code}`
  }
}
</script>