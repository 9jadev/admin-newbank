...
<div class="px-10">
  ...
  <div v-else>
    <div v-if="customers.length == 0" class="bg-[#D2DDFD] flex flex-col h-80 items-center justify-center my-10 rounded-lg w-full">
    ...
    <table v-if="customers.length != 0" class="w-full">
      <thead>
        <tr class="p-6 border-b-2 border-b-[#B8B8B8] bg-[#D2DDFD]">
          ...
          <td class="text-[15px] pl-5 font-semibold leading-[21px] font-[#1D2939] py-4">Name</td> 
          <td class="text-[15px] font-semibold leading-[21px] font-[#1D2939] py-4">Status</td> 
          <td class="text-[15px] font-semibold leading-[21px] font-[#1D2939] py-4">Balance</td> 
      ...
      <tbody>
        <tr  v-for="(item, index) in customers" :key="item.id" class="cusor-auto p-6 border-b-2 hover:bg-[#D2DDFD]">
          <td class="text-[15px] pl-5 font-normal leading-[21px] font-[#1D2939] py-4">{{ index + 1}}</td> 
          <td class="text-[15px] font-bold leading-[21px] font-[#1D2939] py-4">{{ item?.firstname | capitalize }} {{ item?.lastname | capitalize }}</td> 
          <td class="text-[15px] font-normal leading-[21px] font-[#1D2939] py-4">
            <a-tag v-if="item.status == 'active'" color="green">Active</a-tag>
            <a-tag v-if="item.status != 'active'" color="red">{{ item.status | capitalize }}</a-tag>
          </td> 
          <td class="text-[15px] font-bold leading-[21px] font-[#1D2939] py-4">{{ item?.wallet?.balance | currency(item?.currency) }}</td> 
          <td class="text-[15px] font-normal leading-[21px] font-[#1D2939] py-4"> {{ item.created_at | dateFormat }} </td>
          <td class="text-[15px] font-normal leading-[21px] font-[#1D2939] py-4"> 
              <!-- <button @click="showModal(item)" v-if="item.payment_proof == null" class="hover:transition-all hover:bg-[#0042EC] text-white h-5 bg-[#0042EC] focus-visible:outline-none font-normal leading-[150%] rounded-[0.5rem] px-2 text-center">View</button> -->
              <a-button type="primary" @click="routeTo(item.id)"> View </a-button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  data() {
    return {
      ...
      options: "{autosize: true, symbol: BITSTAMP:BTCUSD', interval: 'D',timezone: 'Etc/UTC',theme: 'light',style: '1',locale: 'en',enable_publishing: false,allow_symbol_change: true,container_id: 'tradingview_f8e5a'}"
    }
  },
  methods: {
    routeTo(id) {
    ...
    async listCustomer(params){
      this.loading = true,
			await this.$apiservice.listCustomer().then(res => {
        this.loading = false
