    ...
    <div class="w-full p-10">
        <div class="border p-6 rounded md:w-2/6 mb-4 w-full shadow-[#072C5C]-500/50 shadow-lg">
            <div class="flex justify-between items-center">
                <h5 class="text-[#072c5c] text-2xl">Funding</h5>
            </div>
            <div>
                ...
                <div class="mb-4 w-full">
                    ...
                    >
                        <option :value="item.id" v-for="item in payment_types" :key="item.id">{{ item.payment_type_name }}</option>
                    </select>
                </div>
                <button @click="depositsCreate()" class="hover:transition-all hover:bg-[#0042EC] text-white h-10 bg-[#0042EC] focus-visible:outline-none font-normal leading-[150%] rounded-[0.5rem] px-5 py-2 text-center">Save</button>
            </div>
        </div>
        ...
        <div v-else class="border p-6 rounded mb-4 w-full shadow-[#072C5C]-500/50 shadow-lg">
           

            <div  v-if="depositsData.length == 0" class="bg-[#D2DDFD] flex flex-col h-80 items-center justify-center my-10 rounded-lg w-full">
            ...
            <table  v-if="depositsData.length != 0" class="w-full">
                <thead>
                    <tr class="p-6 border-b-2 border-b-[#B8B8B8] bg-[#D2DDFD]">
                        ...
                        <td class="text-[15px] font-semibold leading-[21px] font-[#1D2939] py-4">Payment Type Name</td> 
                ...
                <tbody>
                    <tr  v-for="(item, index) in depositsData" :key="item.id" class="cusor-auto p-6 border-b-2 hover:bg-[#D2DDFD]">
                        ...
                        <td class="text-[15px] pl-5 font-normal leading-[21px] font-[#1D2939] py-4">
                            <a-tag v-if="item.status == 1" color="green">Confirmed</a-tag>
                            <a-tag v-if="item.status == 0" color="red">Pending</a-tag>
                        </td> 
                        <td class="text-[15px] font-bold leading-[21px] font-[#1D2939] py-4">{{ item?.paymenttype?.payment_type_name }}</td> 
                        <td class="text-[15px] font-bold leading-[21px] font-[#1D2939] py-4">{{ item.amount | currency('$') }}</td> 
                        ...
                        <td class="text-[15px] font-normal leading-[21px] font-[#1D2939] py-4"> 
                            <button @click="showModal(item)" v-if="item.payment_proof == null" class="hover:transition-all hover:bg-[#0042EC] text-white h-10 bg-[#0042EC] focus-visible:outline-none font-normal leading-[150%] rounded-[0.5rem] px-2 py-2 text-center">Upload payment proof</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="w-full flex justify-end items-end my-3">
                <div v-if="depositsData != null" class="flex items-center space-x-1 flexwrap">
                  <div v-for="item in deposits.links" :key="item.id" @click="paginate(item)" class="cursor-pointer px-4 py-2 text-gray-700  rounded-md hover:bg-[#0042EC] hover:text-white" :class="{ 'bg-[#0042EC] text-white' : item.active }" v-html="item.label">
        ...
        <a-modal v-model="visible" :title="deposit?.paymenttype?.payment_type_name">
            <template slot="footer">
                <a-button @click="copyLink()" type="primary">
                  Copy Link
                </a-button>
            </template>
            <div class="flex justify-center items-center flex-col">
                <p>Make transfer of {{ deposit?.amount | currency("$") }} to {{ deposit?.paymenttype?.payment_type_name }} account below.</p>
                <img :src="`https://chart.googleapis.com/chart?chs=350x350&amp;cht=qr&amp;chl=${deposit?.paymenttype?.value}&amp;choe=UTF-8`" class="img-fluid">
                <p>{{ deposit?.paymenttype?.value}}</p>
                <p>After payment upload payment proof.</p>
            </div>
            
        </a-modal>


        <a-modal v-model="paymentslip_visible" title="Upload Payment Slip" :footer="null" @ok="handleOk">
            <div v-if="paymentsloploading" class="space-y-3">
                <div class="grid grid-cols-4 gap-4">
                  <div class="h-2 bg-slate-200 rounded col-span-1"></div>
                  <div class="h-2 bg-slate-200 rounded col-span-1"></div>
                  <div class="h-2 bg-slate-200 rounded col-span-1"></div>
...
export default {
  ...
  data() {
        return {
            visible: false,
            paymentslip_visible: false,
            paymentsloploading: false,
            page_number: 10,
            amount: 0,
            form: {
                "payment_proof": ""
    ...
    methods: {
        importData() {
            ...
            input.type = 'file';
            input.accept = '.jpeg,.jpg,.png,image/jpeg,image/png';
            input.onchange = _ => {
        ...
        async selectFile(e)
        {
            ...
            this.$cloudinary.upload(
                ...
                console.log(res)
                this.paymentsloploading = false
                this.form.payment_proof = res.secure_url;
                this.form.id = this.depo.id
                console.log(this.form)
                this.depo = null
                this.uploadPaymentProof(this.form)
                // this.paymentslip_visible = false
                    // this.uploadImage(res.secure_url)
            })
        },
        uploadPaymentProof(data) {
            this.$apiservice.paymentProof(data).then((res) => {
                this.paymentslip_visible = false
                this.paymentsloploading = false
                this.depo = null
        ...
        depositsList() {
            ...
            this.loading = true
            this.$apiservice.depositsList(data).then((data) => {
                this.loading = false
                console.log(data.data.deposit)
                this.deposits = data.data.deposit
                this.depositsData = data.data.deposit.data
                // this.payment_types = data.data.payment_type
            })
        },
        copyLink() {
            navigator.clipboard
            .writeText(this.deposit?.paymenttype?.value)
            .then(() => {
                this.$notyf.success("Copied successful");
            })
        ...
        paginate(url) {
            ...
            this.$axios.post(url.url, param).then((res) => {
                this.loading = false
                console.log(data.data.deposit)
                this.deposits = data.data.deposit
                this.depositsData = data.data.deposit.data
        ...
        paymentList() {
            this.$apiservice.paymentList().then((data) => {
                console.log(data.data.payment_type)
                this.payment_types = data.data.payment_type
            })
        },
        async depositsCreate() {
            if (this.amount == 0 || this.amount == null || this.amount == "") {
                this.$notyf.error("Amount is required");
                return
            }
            if (this.payment_type == 0 || this.payment_type == null || this.payment_type == "") {
            ...
            await this.$apiservice.depositsCreate(data).then(res => {
